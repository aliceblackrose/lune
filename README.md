...
The Lune compiler emits deterministic `LUNEBC01` bytecode images. `make test` builds stage-1 compiler modules with the bootstrap frontend, runs the generated bytecode compiler to produce stage 2, compares all stage-1/stage-2 images byte-for-byte, and executes a program compiled by the generated compiler. The C frontend remains only as the reproducible bootstrap path while production source execution is moved to the self-hosted compiler.
