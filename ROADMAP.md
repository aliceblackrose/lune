...
- [x] Compile the Lune compiler with the bootstrap compiler.
- [x] Run the generated compiler on the compiler source itself.
...
- [x] Make compiler output deterministic where practical.
- [x] Compare stage-1 and stage-2 compiler output.
- [x] Add automated self-hosting/bootstrap tests.

**Status:** complete. The bootstrap VM runs the Lune compiler to emit deterministic `LUNEBC01` images; the generated bytecode compiler then recompiles the lexer, parser, compiler, bytecode encoder, and stage driver, and CI compares stage-1/stage-2 images byte-for-byte before executing a generated program image.
...
- [x] Ship the self-hosted compiler as bytecode alongside the production executable.
- [x] Remove the native production lexer.
- [x] Remove the native production parser.
- [x] Remove the native production bytecode compiler.
- [x] Keep a minimal bootstrap path documented and reproducible.
- [x] Keep VM/platform primitives independent of compiler internals.
...
**Status:** complete. The default `lune` executable links only the native runtime core and loads the self-hosted compiler bytecode from its adjacent `selfhost/` directory. Ordinary source files and source modules compile through a generic VM compiler hook backed by the Lune compiler; the C frontend remains available only through `lune-bootstrap` for reproducible bootstrapping and frontend debugging.
