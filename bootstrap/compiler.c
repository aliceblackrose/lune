...
struct Compiler {
    ...
    LuneUpvalueDesc upvalues[UPVALUE_MAX];
...
static bool emit_byte(
    ...
    if (lune_chunk_write(compiler->chunk, byte, span)) {
        return true;
    }
...
static bool emit_u16(
    Compiler *compiler,
    uint16_t value,
    LuneSpan span
) {
    return emit_byte(
        compiler, (uint8_t)(value >> 8), span
    ) && emit_byte(
        compiler, (uint8_t)(value & 0xff), span
    );
}
...
static size_t emit_jump(
    ...
    if (!emit_u16(compiler, UINT16_MAX, span)) {
        return SIZE_MAX;
    }
    return operand;
}

static bool patch_jump(
    Compiler *compiler,
    size_t operand,
    LuneSpan span
) {
    if (
        operand == SIZE_MAX ||
        compiler->chunk->count < operand + 2
    ) {
        return false;
    }
    ...
    compiler->chunk->code[operand] =
        (uint8_t)(jump >> 8);
    compiler->chunk->code[operand + 1] =
        (uint8_t)(jump & 0xff);
    return true;
}

static bool emit_loop(
    Compiler *compiler,
    size_t start,
    LuneSpan span
) {
    if (!emit_op(compiler, LUNE_OP_LOOP, span)) {
        return false;
    }

    size_t distance =
        (compiler->chunk->count + 2) - start;
    if (distance > UINT16_MAX) {
        error_at(
            compiler, span, "loop body is too large"
        );
        return false;
    }

    return emit_u16(
        compiler, (uint16_t)distance, span
    );
}

static bool span_equal(
...
static int resolve_local(
    ...
    for (size_t i = compiler->local_count; i > 0; i--) {
        ...
        if (
            local->active &&
            span_equal(compiler, local->name, name)
        ) {
...
static bool local_declared_here(
    const Compiler *compiler,
    LuneSpan name
) {
    for (size_t i = compiler->local_count; i > 0; i--) {
...
static void end_scope(Compiler *compiler) {
    int ending_depth = compiler->scope_depth;
    compiler->scope_depth--;

    for (size_t i = 0; i < compiler->local_count; i++) {
        if (
            compiler->locals[i].active &&
            compiler->locals[i].depth == ending_depth
        ) {
            compiler->locals[i].active = false;
        }
    }
}

static int add_upvalue(
    Compiler *compiler,
    uint16_t index,
    bool is_local,
    LuneSpan span
    ...
    for (size_t i = 0; i < compiler->upvalue_count; i++) {
        if (
            compiler->upvalues[i].index == index &&
    ...
    if (compiler->upvalue_count >= UPVALUE_MAX) {
        error_at(
            compiler,
            span,
    ...
    compiler->upvalues[compiler->upvalue_count] =
        (LuneUpvalueDesc){
            .is_local = is_local,
            .index = index,
        };

    return (int)compiler->upvalue_count++;
}
...
static bool intern_bytes(
    ...
    error_at(
        compiler,
        span,
        "too many strings/names or out of memory"
    );
    return false;
}
...
static bool global_declared(
    const Compiler *compiler,
    uint16_t name
) {
    for (
        size_t i = 0;
        i < compiler->declared_global_count;
        i++
    ) {
        if (compiler->declared_globals[i] == name) {
            return true;
        }
    }
    return false;
...
static bool declare_global(
    ...
    if (
        ...
        size_t next =
            compiler->declared_global_capacity == 0
            ? 8
...
static bool decode_string(
    ...
    if (
        ...
        error_at(
            compiler, span, "invalid string token"
        );
        return false;
    }
    ...
    while (i < end) {
        ...
        if (i >= end) {
            free(decoded);
            error_at(
                compiler, span, "invalid string escape"
            );
            return false;
        }
        ...
        switch (escape) {
            ...
            case 'u': {
                ...
                while (
                    ...
                    int value =
                        hex_value(compiler->source[i++]);

                    if (
                        value < 0 ||
                        codepoint > 0x10ffffu / 16u
                    ) {
                        free(decoded);
                        error_at(
                            compiler,
...
static bool compile_number(
    ...
    if (text == NULL) {
        error_at(
            compiler, node->span, "out of memory"
        );
        return false;
    }
    ...
    if (!lune_chunk_add_constant(
        ...
        error_at(
            compiler,
            node->span,
            "too many constants or out of memory"
        );
        return false;
    }
...
static bool compile_binary(
    ...
    switch (op) {
        ...
        case LUNE_TOKEN_STAR:
            opcode = LUNE_OP_MULTIPLY;
            break;
        case LUNE_TOKEN_SLASH:
            opcode = LUNE_OP_DIVIDE;
            break;
        case LUNE_TOKEN_PERCENT:
            opcode = LUNE_OP_MODULO;
            break;
        case LUNE_TOKEN_EQ:
            opcode = LUNE_OP_EQUAL;
            break;
        case LUNE_TOKEN_NE:
            opcode = LUNE_OP_NOT_EQUAL;
            break;
        case LUNE_TOKEN_LT:
            opcode = LUNE_OP_LESS;
            break;
    ...
    return emit_op(
        compiler, opcode, node->span
    );
}

static bool compile_if(
    Compiler *compiler,
    const LuneAst *node
) {
    if (!compile_node(
    ...
    if (
        !emit_op(
            compiler, LUNE_OP_POP, node->span
        ) ||
        !compile_node(
            compiler,
    ...
    if (
        ...
        compiler, LUNE_OP_NULL, node->span
    )) {
        return false;
    }

    return patch_jump(
        compiler, end_jump, node->span
    );
}

static bool compile_while(
    Compiler *compiler,
    const LuneAst *node
) {
    size_t loop_start =
...
static bool compile_declare(
    Compiler *compiler,
    const LuneAst *node
) {
    LuneSpan name = node->as.declare.name;
    ...
    if (compiler->scope_depth > 0) {
        ...
        if (!compile_node(
            compiler, node->as.declare.value
        )) {
            return false;
        }
    ...
    if (
        !intern_span(
            compiler, name, &global
        ) ||
        !declare_global(
            compiler, global, name
        ) ||
        !compile_node(
            compiler, node->as.declare.value
        )
...
static bool compile_map(
    ...
    for (
        size_t i = 0;
        i < node->as.map.entries.count;
        i++
    ) {
        const LuneMapEntry *entry =
            &node->as.map.entries.items[i];
...
static bool compile_assign(
    Compiler *compiler,
    const LuneAst *node
) {
    const LuneAst *target =
        node->as.assign.target;

    if (target->kind == LUNE_AST_NAME) {
        if (!compile_node(
            compiler, node->as.assign.value
        )) {
            return false;
        }

        int local = resolve_local(
            compiler, target->span
        );
        if (local >= 0) {
            return emit_indexed(
                compiler,
...
static bool compile_function(
    ...
    LuneFunction *function = lune_function_new(
        (uint16_t)node->as.function.parameters.count
    );
    if (function == NULL) {
        error_at(
            compiler, node->span, "out of memory"
    ...
    for (
        size_t i = 0;
        i < node->as.function.parameters.count;
        i++
    ) {
        LuneSpan parameter =
    ...
    bool ok = compile_node(
        &child, node->as.function.body
    );
    ...
    if (!lune_chunk_add_function(
        ...
        error_at(
            compiler,
            node->span,
            "too many functions or out of memory"
        );
        return false;
    }
...
static bool compile_call(
    ...
    return emit_indexed(
        compiler,
        LUNE_OP_CALL,
        (uint16_t)node->as.call.arguments.count,
        node->span
    );
}
...
static bool compile_node(
    ...
    switch (node->kind) {
        ...
        case LUNE_AST_STRING:
            return emit_string_span(
                compiler, node->span, true
            );
