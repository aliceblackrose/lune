...
static LuneObj *allocate_object(LuneHeap *heap, size_t size, LuneObjKind kind) {
...
LuneObjList *lune_list_new(LuneHeap *heap, const LuneValue *items, size_t count) {
    ...
    if (count > 0) {
        if (count > SIZE_MAX / sizeof(*list->items)) return NULL;
        list->items = malloc(count * sizeof(*list->items));
        if (list->items == NULL) return NULL;
        memcpy(list->items, items, count * sizeof(*list->items));
    }
    list->count = count;
    return list;
}
...
LuneObjMap *lune_map_new(LuneHeap *heap) {
    return (LuneObjMap *)allocate_object(heap, sizeof(LuneObjMap), LUNE_OBJ_MAP);
}
...
static bool string_equal_chars(
    ...
    return string->length == length && memcmp(string->chars, chars, length) == 0;
}
...
bool lune_map_get(
    ...
    return lune_map_get_chars(map, key->chars, key->length, value);
}
...
bool lune_map_set(
    ...
    (void)heap;
    for (size_t i = 0; i < map->count; i++) {
        if (string_equal_chars(map->entries[i].key, key->chars, key->length)) {
...
bool lune_map_set_chars(
    ...
    LuneObjString *key = lune_string_new(heap, chars, length);
    if (key == NULL) return false;
    return map_append(map, key, value);
}

bool lune_obj_is_string(const LuneObj *object) {
    return object != NULL && object->kind == LUNE_OBJ_STRING;
}

bool lune_obj_is_list(const LuneObj *object) {
    return object != NULL && object->kind == LUNE_OBJ_LIST;
}

bool lune_obj_is_map(const LuneObj *object) {
    return object != NULL && object->kind == LUNE_OBJ_MAP;
}

static void print_object(FILE *out, const LuneObj *object, unsigned depth);

static void print_value(FILE *out, LuneValue value, unsigned depth) {
    switch (value.kind) {
...
static void print_object(FILE *out, const LuneObj *object, unsigned depth) {
    ...
    switch (object->kind) {
        case LUNE_OBJ_STRING: {
            const LuneObjString *string = (const LuneObjString *)object;
            fwrite(string->chars, 1, string->length, out);
            break;
        }
        case LUNE_OBJ_LIST: {
            const LuneObjList *list = (const LuneObjList *)object;
            fputc('[', out);
            for (size_t i = 0; i < list->count; i++) {
                if (i != 0) fputs(", ", out);
                print_value(out, list->items[i], depth + 1);
            }
            fputc(']', out);
            break;
        }
        case LUNE_OBJ_MAP: {
            const LuneObjMap *map = (const LuneObjMap *)object;
            fputc('{', out);
            for (size_t i = 0; i < map->count; i++) {
                if (i != 0) fputs(", ", out);
                fwrite(map->entries[i].key->chars, 1, map->entries[i].key->length, out);
                fputs(": ", out);
                print_value(out, map->entries[i].value, depth + 1);
            }
            fputc('}', out);
            break;
        }
